...
- **`#plans`** — `section--pale`. Plans are sold as **monthly memberships, billed every 4 weeks** (per-session rate unchanged from the old cycle-based model). Three-tier `.plan-grid > .plan-card` + `.single-strip` standalone:
  ...
  - **Online consultancy** — third `.single-strip` below the assessment. A$100 every 2 months, auto-renewing, for online check-ins with Marina to review progress and adjust the training program. CTA currently points to a WhatsApp deep link (`wa.me/61451021478?text=...`) as a placeholder — swap to an Acuity URL with `acuity-embed-button` once Marina creates the appointment-type (suggested slug `/OnlineConsultancy`). Keys `tr.consult.{name,sub,price,cta}`.
