...
- `*.webp` / `*.jpeg` / `*.jpg` / `*.png` / `*.svg` — image assets live flat in the repo root and are referenced by bare filename (no `assets/` subfolder). Currently in-use brand imagery:
  - `hero.png` — main brand portrait used as the home page hero image (`hero.hero__media`), `og:image` for `index.html` and `method.html`, and the `LocalBusiness.image` in JSON-LD.
...
- `<link rel="preload" as="image" href="<hero-img>" fetchpriority="high">` for the page's LCP image (home: `hero.png`; about: `marina-hero.webp`; other inner pages: none — they don't have a hero image).
