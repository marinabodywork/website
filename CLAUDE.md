...
- **`#mas-memberships`**, `section--dark`, sits between `#mas-pricing` and the FAQ. Prepaid **session-pack memberships** (distinct from the weekly/Ultimate recurring memberships above and from the PT plans on `training.html`). Two `.gem-card` tiers in a `.gem-grid` (2-up, collapses to 1 under 640px): **Diamond** (20 sessions · A$98/session · A$1,960 upfront · access to all treatments · worth A$2,500, saves A$540) and **Emerald** (`.gem-card--emerald`, 10 sessions · A$107/session · A$1,070 upfront · choose 2 of the available services · worth A$1,250, saves A$180). Each card carries a `.gem-card__worth` value pill and a `.gem-card__meta` line (valid 12 weeks · suits 2×/week or splitting the package). Below: a `.gem-benefits` list (priority booking / member rates / personalised journey / offers + gifts), a `.gem-pay` note (**payment by bank transfer or cash** — no Acuity), a `.gem-scarcity` line, and a single WhatsApp CTA (`wa.me/61451021478`, "Secure your place →"). **No Acuity/online checkout** — the whole flow is arranged over WhatsApp, so the CTA is intentionally not an `acuity-embed-button`. Keys `mas.gem.*`. Emerald's A$1,070 upfront total is registered in `check-pricing.mjs` and obeys the cabalistic rule. **Diamond's A$1,960 upfront is an intentional exception to the cabalistic rule (sums to 16), kept at Marina's request** — it is deliberately left out of `check-pricing.mjs`'s `HEADLINE_PRICES` (see the comment there); don't "fix" it without confirming with Marina.
