...
- **`#plans`** — `section--pale`. Three-tier `.plan-grid > .plan-card` + `.single-strip` standalone:
  - **Basic** — A$570, A$95/session · 6 sessions · 1x/week · 45-day cycle · freeze 1 week · MFIT app included. URL `/PTBasic`.
  - **Golden** — `.plan-card__badge` "Most popular". A$1,020, A$85/session · 12 sessions · 2x/week · 2-month cycle · freeze 2 weeks · MFIT app included. URL `/PTGolden`.
  - **Diamond** — `.plan-card__badge--alt` "Best value". A$1,800, A$75/session · 24 sessions · 3x/week · 3-month cycle · freeze 3 weeks · MFIT app included. URL `/DiamondPTSession`.
  - **Single session** — `.single-strip`. A$99, no commitment. URL `/SinglePTLesson`.
  - All plan CTAs carry `acuity-embed-button`. `tr.plans.sub` and `tr.plan.note` both explain the "first session's day+time becomes your fixed weekly slot for the cycle" rule.
