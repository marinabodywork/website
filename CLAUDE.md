...
- **`#plans`** — `section--pale`. Plans are sold as **monthly memberships, billed every 4 weeks** (per-session rate unchanged from the old cycle-based model). Three-tier `.plan-grid > .plan-card` + `.single-strip` standalone:
  - **Basic** — A$380/month (A$95/session × 4 sessions/month) · 1x/week, same weekday + time · **2-month minimum** · freeze 1 week per minimum period · MFIT app included. URL `/PTBasic`.
  - **Golden** — `.plan-card__badge` "Most popular". A$680/month (A$85/session × 8 sessions/month) · 2x/week, same weekdays + times · **2-month minimum** · freeze 2 weeks per minimum period · MFIT app included. URL `/PTGolden`.
  - **Diamond** — `.plan-card__badge--alt` "Best value". A$900/month (A$75/session × 12 sessions/month) · 3x/week, same weekdays + times · **3-month minimum** · freeze 3 weeks per minimum period · MFIT app included. URL `/DiamondPTSession`.
