...
- **`#plans`** — `section--pale`. Three-tier `.plan-grid > .plan-card` + `.single-strip` standalone:
  - **Basic** — A$570, A$95/session · 6 sessions · 1x/week · 6-week cycle · freeze 1 week · MFIT app included. URL `/PTBasic`.
  - **Golden** — `.plan-card__badge` "Most popular". A$1,020, A$85/session · 12 sessions · 2x/week · 6-week cycle · freeze 2 weeks · MFIT app included. URL `/PTGolden`.
  - **Diamond** — `.plan-card__badge--alt` "Best value". A$1,800, A$75/session · 24 sessions · 3x/week · 8-week cycle · freeze 3 weeks · MFIT app included. URL `/DiamondPTSession`.
...
- **`#policy`** — `section--cream`, `container--reading`. "Package policy — rescheduling, renewal, and pause." Five `.policy-block` subsections (`tr.pol.*` keys): your weekly slot, rescheduling (with a `.policy-list` ul of the four edge cases — <72h reschedule, no-show, calendar full, Marina cancels), auto-renewal, emergency freeze (1/2/3 weeks per Basic/Golden/Diamond), single sessions. Everything is self-serve through the client's Acuity portal — the rules here mirror what's actually configured in Acuity admin (72h reschedule window, 7-day pre-renewal cancellation window, per-plan freeze entitlement). Edits to these rules need to be made in **both** the visible copy and Acuity admin so they stay in lockstep.
...
**Recurring-booking instructions live in two places and must stay in lockstep.** The PT plans are sold without auto-booked sessions — the client has to choose **"Select and make recurring"** themselves on their first booking (and repeat once per training day for Golden/Diamond). The wording `Select and make recurring` appears verbatim in `tr.plans.sub`, `tr.plan.note`, and `tr.pol.slot.p` (EN + PT) so clients reading the site know exactly which button to click, AND in the **Acuity appointment-type Description field** for `PTBasic` / `PTGolden` / `DiamondPTSession` so clients who land on Acuity directly (via WhatsApp, email reminders, bookmarks) see the same instruction above the calendar. If Acuity ever renames that button, update both surfaces. If you change the per-plan series count (Basic 1 / Golden 2 / Diamond 3) or per-series occurrences (6 / 6 / 8 — which match the per-plan cycle length in weeks), update the visible copy, the Acuity descriptions, and the Acuity appointment-type "max recurrences" setting all together.
