...
- **`#plans`** — `section--pale`. Plans are sold as **monthly memberships, billed every 4 weeks** (per-session rate unchanged from the old cycle-based model). Three-tier `.plan-grid > .plan-card` + `.single-strip` standalone:
  - **Basic** — A$380/month (A$95/session × 4 sessions/month) · 1x/week, same weekday + time · **2-month minimum** · freeze 1 week per minimum period. Live sessions only — no MFIT app, no physical assessment. URL `/PTBasic`.
  - **Golden** — `.plan-card__badge` "Most popular". A$680/month (A$85/session × 8 sessions/month) · 2x/week, same weekdays + times · **2-month minimum** · freeze 2 weeks per minimum period · MFIT app programming included (no physical assessment). URL `/PTGolden`.
  - **Diamond** — `.plan-card__badge--alt` "Best value". A$900/month (A$75/session × 12 sessions/month) · 3x/week, same weekdays + times · **3-month minimum** · freeze 3 weeks per minimum period · MFIT app + **physical assessment** both included. URL `/DiamondPTSession`.
  ...
  - **Physical assessment** — second `.single-strip` directly below the single session. A$99 standalone 60-minute movement/posture/strength assessment with Marina. Included with Diamond, optional one-off for Basic/Golden clients (or anyone curious). CTA currently uses a WhatsApp deep link (`wa.me/61451021478?text=...`) because Marina does not yet have a dedicated Acuity appointment-type for assessments — swap to an Acuity URL with `acuity-embed-button` when that's set up. Keys `tr.assess.{name,sub,price,cta}`.
...
**Membership model details (kept in lockstep with Acuity).** Plans are subscription products billed every 4 weeks; the per-session rate is unchanged from the old package model (A$95 / A$85 / A$75 for Basic / Golden / Diamond). Minimum commitment is 2 months on Basic and Golden, 3 months on Diamond. Cancellation inside the minimum is **transfer-to-a-friend, no refund** — the visible copy in `tr.pol.commit.*` and `tr.plan.note` mirrors what Acuity admin enforces. After the minimum, clients cancel themselves through the Acuity portal with at least 7 days notice before the next billing date. The recurring-booking wording (`Select and make recurring`) appears verbatim in `tr.plans.sub`, `tr.plan.note`, and `tr.pol.slot.p` (EN + PT) AND in the **Acuity appointment-type Description field** for `PTBasic` / `PTGolden` / `DiamondPTSession` so clients who land on Acuity directly see the same instruction above the calendar. If Acuity ever renames that button, update both surfaces. The per-plan series count stays the same (Basic 1 / Golden 2 / Diamond 3 weekly series) — what changed is that recurrences run for the length of the membership, not a fixed cycle.
